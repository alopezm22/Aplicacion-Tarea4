...
fun Navigation() {
    ...
    NavHost(navController = navController, startDestination = AuthManager.toString()) {
        ...
        composable(SecondPage::class.java.name + "/{id}") { backStackEntry ->
            val args = backStackEntry.arguments?.getString("id")?.toInt() ?: 0
            SecondScreen(navController, args)
        }

        composable(TerceraPage::class.java.name + "/{id}") { backStackEntry ->
            val args = backStackEntry.arguments?.getString("id")?.toInt() ?: 0
            TerceraScreen(navController, args)
        }

        composable(FourPage::class.java.name + "/{id}") { backStackEntry ->
            val args = backStackEntry.arguments?.getString("id")?.toInt() ?: 0
            FourScreen(navController, args)
        }
    }
}
