...
import com.bfernandez.api_rest.R
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                ...
                actions = {
                    Button(
                        onClick = { navController.navigate(CategoriesList) },
                    ...
                    Button(
                        onClick = { navController.navigate(TerceraPage) },
                        contentPadding = PaddingValues(horizontal = 12.dp),
                        colors = ButtonDefaults.buttonColors(
                            contentColor = Color.White,
                            containerColor = Color(0xFFE50914),
        ...
        when (val state = viewModel.generosui) {
            is GenerosUi.Loading -> {
                Text(text = "Cargando géneros...", modifier = Modifier.padding(innerPadding))
            }
            is GenerosUi.Error -> {
                Text(text = "Error: ${state.message}", modifier = Modifier.padding(innerPadding))
            }
